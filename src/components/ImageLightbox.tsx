...
const ImageLightbox = ({ src, alt, onClose }: ImageLightboxProps) => {
  ...
  return createPortal(
    <AnimatePresence>
      <motion.div
        className="fixed inset-0 z-[100] overflow-y-auto bg-black/85 backdrop-blur-sm cursor-zoom-out"
        onClick={onClose}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="fixed top-4 right-4 z-[101] flex h-10 w-10 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white hover:bg-white/20 transition-colors"
